...
def capture_image(step_count, robot_id):
    ...
    filename = os.path.join(image_dir, f"terrain2_step_{step_count}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
    plt.imsave(filename, img)
    print(f"Captured and saved image at step {step_count}: {filename}")


# Terrain creation function
def create_terrain_2():
...
class TestudogEnv(gym.Env):
    ...
    def init_state(self):
        ...
        p.resetSimulation()
        heightfield=True
        create_terrain_2()  # Create uneven terrain
...
if __name__ == '__main__':
    ...
    # Load model
    model_path = f"{model_dir}/PPO_terrainTwo_final_II_309"
