...
def create_terrain_6():
    ...
    heightfield_data = np.random.uniform(-1, 1, 256 * 256)
    max_height_index = np.argmax(heightfield_data)
    peak_x = (max_height_index % 256) * 1.0  # Adjusting for mesh scale
    peak_y = (max_height_index // 256) * 1.0
    peak_z = heightfield_data[max_height_index] * 1.0  # Adjust for height scale

    terrain_shape = p.createCollisionShape(
        shapeType=p.GEOM_HEIGHTFIELD,
        meshScale=[1.0, 1.0, 0.0],
    ...
    # Return the terrain and the peak position as a tuple
    return terrain, (peak_x, peak_y, peak_z)
...
class TwoWheelRobot:
    def __init__(self,
        ...
        self.peak_position = (1.15, 1.15, 0.3)
        self.reset()
    ...
    def reset(self):
        ...
        if self.flat_env == 'SLOPE':
            self.terrain, self.peak_position = create_terrain_6()  # Store the peak position here
            self.goal_position = self.peak_position  # Optionally set goal position to the peak
            self.planeid = self.terrain
        else:
            # Load plane only if environment type is 'FLAT'
            urdfRootPath = pybullet_data.getDataPath()
            self.planeid = p.loadURDF(os.path.join(urdfRootPath, "plane.urdf"), basePosition=[0, 0, 0])

        self.goal_position = (self.x_distance_to_goal, 0, 0)  # Default for flat terrain
    ...
    def check_terminal_state(self, time_step, goal_step):
        ...
        elif self.goal_type == 'peak':
            # Calculate distance to the peak position
            peak_x, peak_y, peak_z = self.peak_position  
            distance_to_peak = ((x - peak_x) ** 2 + (y - peak_y) ** 2 + (z - peak_z) ** 2) ** 0.5
            peak_threshold = 1.0  # Define a threshold distance to consider reaching the peak
