...
def create_terrain_5():
    p.setAdditionalSearchPath(pybullet_data.getDataPath())
    terrain_shape = p.createCollisionShape(
        shapeType=p.GEOM_HEIGHTFIELD,
        meshScale=[1.15, 1.15, 0.2],
    ...
    p.changeDynamics(terrain, -1, lateralFriction=1.75)
...
class TestudogEnv(gym.Env):
    ...
    def init_state(self):
        ...
        p.resetSimulation()
        heightfield=True
        create_terrain_5()  # Create uneven terrain
        p.setRealTimeSimulation(0)
        p.setAdditionalSearchPath(pybullet_data.getDataPath())
        self.testudogid = p.loadURDF("C:/Users/thapelo/Downloads/Quadruped_Robot/Quadruped_Robot/urdf/testudog.urdf", [0, 0, 0.4], [0, 0, 0, 1])
    ...
    def step(self, action):
        ...
        p.setJointMotorControlArray(self.testudogid, list(range(12)), p.POSITION_CONTROL,
                                    ...
                                    positionGains=[0.06, 0.06, 0.06,  # Higher for front-left leg (Leg 0)
                                                   0.06, 0.06, 0.06,  # Higher for front-right leg (Leg 1)
                                                   0.04, 0.04, 0.04,  # Lower for back-left leg (Leg 2)
                                                   0.04, 0.04, 0.04],  # Lower for back-right leg (Leg 3)
                                    velocityGains=[0.4, 0.4, 0.4,  # Higher velocity gain for front-left (Leg 0)
                                                   0.4, 0.4, 0.4,  # Higher velocity gain for front-right (Leg 1)
                                                   0.25, 0.25, 0.25,  # Lower velocity gain for back-left (Leg 2)
                                                   0.25, 0.25, 0.25])  # Lower velocity gain for back-right (Leg 3)
        ...
        reward -= 20 * abs(body_pos[0])  # Stronger penalty for sideways movement
        ...
        #Momentum control
        # Penalize large sudden velocity changes to control momentum
        velocity_change = abs(body_lin_vel[0] - self.previous_velocity)
        reward -= 10 * velocity_change  # Penalize sharp velocity changes
        # Penalize backward movement (negative velocity)
        if body_lin_vel[0] < 0:
            reward -=100 * abs(body_lin_vel[0]) # Penalty for moving backward
        if body_lin_vel[0] > 0 and abs(velocity_change) < 0.1:
            reward += 50  # Reward consistent forward movement with minimal velocity changes
        # Update the previous velocity for the next step
        self.previous_velocity = body_lin_vel[0]
        # Dynamically adjust velocity gains based on movement direction (Dampen Oscillations Using Velocity Gains:)
        if body_lin_vel[0] < 0:
            # Reduce velocity gain when sliding back
            vel1 *= 0.8
            vel2 *= 0.8
        else:
            # Increase velocity gain for forward movement
            vel1 *= 1.2
            vel2 *= 1.2
...
if __name__ == '__main__':
    ...
    TIMESTEPS = 100000  # Number of timesteps per training iteration
    learning_rate = 0.0005  # New learning rate to apply

    # Load pre-trained model if available
    model_path = f"{model_dir}/PPO_terrainFIVE_IV_320.zip"  # Adjust the path topre-trained model
    ...
    try:
        while True:
            ...
            model_save_path = f"{model_dir}/PPO_terrainFIVE_Xl_{count}.zip"
