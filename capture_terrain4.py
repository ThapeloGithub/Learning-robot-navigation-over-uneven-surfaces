...
def capture_image(step_count, robot_id):
    ...
    filename = os.path.join(image_dir, f"terrain4_step_{step_count}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
    plt.imsave(filename, img)
    print(f"Captured and saved image at step {step_count}: {filename}")


# Terrain creation function
def create_terrain_4():
    p.setAdditionalSearchPath(pybullet_data.getDataPath())
    terrain_shape = p.createCollisionShape(
        shapeType=p.GEOM_HEIGHTFIELD,
        meshScale=[0.1, 0.1, 0.15],
    ...
    p.changeDynamics(terrain, -1, lateralFriction=0.9)
...
class TestudogEnv(gym.Env):
    ...
    def init_state(self):
        ...
        p.resetSimulation()
        heightfield=True
        create_terrain_4()  # Create uneven terrain
        p.setRealTimeSimulation(0)
        p.setAdditionalSearchPath(pybullet_data.getDataPath())
        self.testudogid = p.loadURDF("C:/Users/thapelo/Downloads/Quadruped_Robot/Quadruped_Robot/urdf/testudog.urdf", [0, 0, 0.3], [0, 0, 0, 1])
    ...
    def step(self, action):
        ...
        p.setJointMotorControlArray(self.testudogid, list(range(12)), p.POSITION_CONTROL,
                                    ...
                                    positionGains=[0.06, 0.06, 0.06, # Higher for front-left leg (Leg 0)
                                                   ...
                                                   0.05, 0.05, 0.05, # Lower for back-left leg (Leg 2)
                                                   0.05, 0.05, 0.05],  # Lower for back-right leg (Leg 3)
                                    velocityGains=[0.4, 0.4, 0.4, # Higher velocity gain for front-left (Leg 0)
                                                   ...
                                                   0.4, 0.4, 0.4, # Lower velocity gain for back-left (Leg 2)
                                                   0.4, 0.4, 0.4])  # Lower velocity gain for back-right (Leg 3)
        focus, _ = p.getBasePositionAndOrientation(self.testudogid)
        p.resetDebugVisualizerCamera(cameraDistance=1, cameraYaw=-90, cameraPitch=0, cameraTargetPosition=focus)
        p.stepSimulation()
...
if __name__ == '__main__':
    ...
    # Load model
    model_path = f"{model_dir}/PPO_terrainFOUR_final_309"
